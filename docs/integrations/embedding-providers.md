...
| `openrouter` (**one key for everything** — embeddings + `rerank-2.5` reranker + chat, see below) | `OPENROUTER_API_KEY` | per-model (1024 for the canonical `voyageai/voyage-4`; 1536 for `openai/text-embedding-3-small`; unlisted ids require explicit dims) | 0.06 (`voyageai/voyage-4`, Voyage's own rate) | no | model-dependent |
...
- **Reranking pair**: Voyage `rerank-2.5` ($0.05/M; `rerank-2.5-lite` at $0.02/M for cost-sensitive setups) is the mode-bundle default and rides the same `VOYAGE_API_KEY` as embeddings. The default is key-aware: with no Voyage key but an OpenRouter key present it becomes `openrouter:voyageai/rerank-2.5` (same cross-encoder, same key as your OpenRouter embeddings) — `gbrain search modes` and `gbrain doctor` say so; with neither key search fails open in fusion order and both surfaces name the fix. ZeroEntropy `zerank-2` is deprecated (hosted API ends 2026-09-04); an explicitly configured `zeroentropyai:zerank-*` short-circuits past that date (see [`docs/ai-providers/zeroentropy.md`](../ai-providers/zeroentropy.md)).
...
- **One key for everything**: OpenRouter. With `OPENROUTER_API_KEY` as your only key, `gbrain init --pglite` (no flags) lands on `voyageai/voyage-4` embeddings @ 1024d, the `voyageai/rerank-2.5` reranker, and `anthropic/*` chat/extraction — all proxied by OpenRouter, nothing pinned. Any `openrouter:<provider>/<model>` still works explicitly for chat against GPT-5.2, Claude 4.x, Gemini 3, DeepSeek, and dozens more; the embedding catalog also includes OpenAI, Google, Qwen, BGE-M3.
...
gbrain init --pglite          # → openrouter:voyageai/voyage-4 @ 1024d; reranker: openrouter:voyageai/rerank-2.5 (nothing written)
gbrain doctor                 # green — reranker ready, fact extraction keyed
gbrain import ./some-repo
gbrain search "..." --explain # rerank stage present, attributed to openrouter
gbrain think "..."            # synthesized via openrouter:anthropic/claude-sonnet-4.6
```
...
**Embedding**: the canonical pick is `voyageai/voyage-4` (1024d — the same embedding space as the native new-install default, so a later move to a direct Voyage key needs no reindex); `voyageai/voyage-4-large` / `voyageai/voyage-4-lite` (also 1024d) share that space. OR's vendor slug is `voyageai/` — `voyage/voyage-4` does not exist there (HTTP 400) and is deliberately unlisted so a typo can't plan a column. The catalog also proxies `openai/text-embedding-3-small` (1536d, Matryoshka shrink to 512/768/1024), `openai/text-embedding-3-large` (3072), `qwen/qwen3-embedding-8b` (4096), `bge-m3` (1024) — opting in via `--embedding-model openrouter:<id>` plans the right column width automatically. Any id NOT in that list (including `google/gemini-embedding-2-preview`, whose width is unverified) has no silent default: you must pass explicit dimensions (`--embedding-dimensions <N>` or `embedding_dimensions` config) or the command errors with the fix. Pricing matches the upstream provider (`voyageai/voyage-4` bills Voyage's $0.06/M). Note that OR's public `/api/v1/models` catalog does not list embedding or rerank models; the recipe's ids were verified against the live endpoints.

**Reranker**: `voyageai/rerank-2.5` and `voyageai/rerank-2.5-lite` are allowlisted alongside Cohere v3.5/4 and NVIDIA Nemotron. When `OPENROUTER_API_KEY` is your only key, the mode-bundle reranker default resolves to `openrouter:voyageai/rerank-2.5` automatically (`gbrain search modes` attributes it as the key-aware mode default; `gbrain doctor`'s `reranker_health` is green) — no `search.reranker.model` row needed. To pick a different OR reranker explicitly: `gbrain config set search.reranker.model openrouter:cohere/rerank-v3.5`.

**Chat**: every chat model OR proxies works through `/v1/chat/completions`. The recipe lists 8 curated entry points (GPT-5.2 family, Claude 4.5/4.6/4.7, Gemini 3 Flash Preview, DeepSeek); any other OR catalog ID also works. Tool-calling envelope is supported by the OR endpoint, but per-model capability varies — check https://openrouter.ai/models before counting on tools for a specific slug. When OpenRouter is the only chat-capable key, the key-aware tier defaults route chat-shaped work through OR's Anthropic route with no `models.*` pins: utility → `openrouter:anthropic/claude-haiku-4.5`, reasoning → `openrouter:anthropic/claude-sonnet-4.6`, deep → `openrouter:anthropic/claude-opus-4.7`, subagent → `openrouter:anthropic/claude-sonnet-4.6` (one constant, `OPENROUTER_TIER_DEFAULTS` in `src/core/model-config.ts`). Override any tier the usual way: `gbrain config set models.tier.reasoning openrouter:google/gemini-3-flash-preview`.
...
**Subagent loops**: the gateway tool loop keys crash-replay on gbrain's own `gbrainToolUseId`, not the provider's ids, so what matters per OR family is that its tool-call envelope survives an abort mid-loop and a resume. The `anthropic/` and `deepseek/` families have live abort/retry replay pins (`test/e2e/openrouter-*-subagent-replay.live.test.ts`) and are accepted at submit time; the subagent handler auto-routes them through the gateway loop (no `agent.use_gateway_loop` flag needed), so an OpenRouter-only brain runs `gbrain dream`, `gbrain agent run` and `gbrain autopilot` on `openrouter:anthropic/claude-sonnet-4.6` without a direct Anthropic key. Other OR families are refused at submit with a message naming the fix until they get their own pin.
