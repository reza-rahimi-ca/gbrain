...
API keys live in `~/.gbrain/config.json` (file plane) or env vars (`VOYAGE_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY`). Set them via env, or with `gbrain config set <KEY> <value>` — vendor API keys (and `database_url`/`database_path`) are file-plane routed, so the write lands where the pipeline actually reads it. Either spelling of a provider key works: `gbrain config set OPENROUTER_API_KEY sk-or-...` (the env-var name) and `gbrain config set openrouter_api_key sk-or-...` both write the `openrouter_api_key` field the runtime reads, redacted as `***` on read-back. Every surface that asks "is this key present?" — `providers explain`, `doctor`, `search modes`, init's provider detection, the model resolver — reads the same fold (env over config.json), so a key set only in config.json counts everywhere:
...
With **only** `OPENROUTER_API_KEY` set, `gbrain init --pglite` needs no flags: embeddings land on `openrouter:voyageai/voyage-4` @ 1024d (the same embedding space as the native default, so a later move to a direct Voyage key needs no reindex), the reranker default resolves to `openrouter:voyageai/rerank-2.5` on the same key, and every chat-shaped feature routes to `openrouter:anthropic/*` — nothing is written to `models.*`, and init persists no `expansion_model` / `chat_model` pin either (the chat tiers resolve at runtime from whichever key is present, so switching keys later needs no config edit). OpenRouter is the auto-pick only when it is the sole key; a native Voyage/Anthropic/OpenAI key alongside it keeps winning. Details in [`docs/integrations/embedding-providers.md`](integrations/embedding-providers.md) → OpenRouter.
