...
describe('root OpenClaw plugin manifest', () => {
  it('declares the id required by OpenClaw plugin installs', () => {
    ...
    const entryId = entrySource.match(/PLUGIN_ID = '([^']+)'/)?.[1];

    expect(manifest.id).toBe(entryId);
    expect(manifest.configSchema).toBeDefined();
    expect(typeof manifest.configSchema).toBe('object');
    // The entry binds the engine under the legacy engine id AND the plugin id
    // (OpenClaw 2026.9+ resolves the slot by plugin id); declare both.
